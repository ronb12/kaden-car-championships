...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && body.type === 'leave') {
      ...
      await sql`
        INSERT INTO krc_presence (
          ...
          x, z, angle, speed_kmh, lap, progress, updated_at
        )
        VALUES (
          ...
          ${Number(body.x) || 0}, ${Number(body.z) || 0}, ${Number(body.angle) || 0},
          ${Number(body.speedKmh) || 0}, ${parseInt(body.lap, 10) || 1}, ${Number(body.progress) || 0}, NOW()
        )
    ...
    const players = await sql`
      SELECT player_id, player_name, car_name, car_id, color_int, x, z, angle, speed_kmh, lap, progress
      FROM krc_presence
