...
(function (global) {
  ...
  function tintModel(root, bodyHex) {
    root.traverse(function (o) {
      ...
      mats.forEach(function (m) {
        ...
        if (/body|paint|helga|shell|exterior|t_body/.test(n) && m.color) {
          m.color.setHex(bodyHex);
          if (m.emissive) m.emissive.setHex(bodyHex);
          if (m.emissiveIntensity !== undefined) m.emissiveIntensity = 0.22;
          if (m.metalness !== undefined) m.metalness = 0.42;
          if (m.roughness !== undefined) m.roughness = 0.36;
  ...
  function previewBodyStyle(carId) {
    var id = String(carId || 'f40');
    if (id === 'police') return 'police';
    if (['bugatti', 'koenigsegg', 'jesko', 'senna', 'mclaren'].indexOf(id) >= 0) return 'hyper';
    if (['lambo', 'huracan'].indexOf(id) >= 0) return 'exotic';
    if (['f40', 'corvette', 'nsx', 'audi-r8'].indexOf(id) >= 0) return 'super';
    if (['gtr', 'supra', 'rx7', '911', 'm4', 'r34', 'r35-nismo'].indexOf(id) >= 0) return 'sports';
    if (['mustang', 'camaro', 'challenger', 'charger', 'viper', 'gt500'].indexOf(id) >= 0) return 'muscle';
    if (['evo', 'sti', 'civic'].indexOf(id) >= 0) return 'tuner';
    if (['amg', 'rs7'].indexOf(id) >= 0) return 'sedan';
    if (id === 's2000') return 'roadster';
    return 'sports';
  }

  function previewSilhouetteDims(carId) {
    switch (previewBodyStyle(carId)) {
      case 'hyper': return { x: 1.88, y: 0.9, z: 4.62 };
      case 'exotic': return { x: 2.12, y: 0.96, z: 4.44 };
      case 'super': return { x: 1.94, y: 0.9, z: 4.32 };
      case 'muscle': return { x: 2.14, y: 0.98, z: 4.68 };
      case 'tuner': return { x: 1.9, y: 0.88, z: 4.08 };
      case 'sedan': return { x: 2.02, y: 0.96, z: 4.78 };
      case 'roadster': return { x: 1.84, y: 0.8, z: 3.92 };
      case 'police': return { x: 2.1, y: 1.0, z: 4.72 };
      default: return { x: 1.96, y: 0.9, z: 4.26 };
    }
  }

  function fitModel(model, carId) {
    var box = new global.THREE.Box3().setFromObject(model);
    var size = new global.THREE.Vector3();
    box.getSize(size);
    var span = Math.max(size.x, size.z);
    if (span < size.y * 0.75) span = Math.max(size.x, size.y, size.z, 0.001);
    var s = 3.6 / span;
    model.scale.setScalar(s);
    var ref = { x: 1.94, y: 0.9, z: 4.5 };
    var d = previewSilhouetteDims(carId);
    model.scale.x *= d.x / ref.x;
    model.scale.y *= d.y / ref.y;
    model.scale.z *= d.z / ref.z;
